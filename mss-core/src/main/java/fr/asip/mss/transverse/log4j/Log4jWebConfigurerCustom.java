...
/**
 * Convenience class that performs custom log4j initialization for web
 * environments, allowing for log file paths within the web application, with
 * the option to perform automatic refresh checks (for runtime changes in
 ...
 * 
 * <p>
 * Supports three init parameters at the servlet context level (that is,
 ...
 * "classpath:myLog4j.properties"), an absolute file URL (e.g.
 * "file:C:/log4j.properties), or a plain path relative to the web application
 * root directory (e.g. "/WEB-INF/log4j.properties"). If not specified, default
 * log4j initialization will apply ("log4j.properties" or "log4j.xml" in the
 * class path; see the log4j documentation for details).
 ...
 * activity (when using log4j), for proper initialization before any Spring
 * logging attempts.
 * 
 * <p>
 * Log4j's watchdog thread will asynchronously check whether the timestamp of
 * the config file has changed, using the given interval between checks. A
 * refresh interval of 1000 milliseconds (one second), which allows to do
 * on-demand log level changes with immediate effect, is not unfeasible.
 * 
 * <p>
 * <b>WARNING:</b> Log4j's watchdog thread does not terminate until VM shutdown;
 ...
 * environment; the watchdog thread would not stop on application shutdown
 * there.
 * 
 * <p>
 * By default, this configurer automatically sets the web app root system
 * property, for "${key}" substitutions within log file locations in the log4j
 ...
 * log4j config file like as follows:
 * 
 * <p>
 * <code>log4j.appender.myfile.File=${webapp.root}/WEB-INF/demo.log</code>
 * 
 * <p>
 ...
 * 
 * <p>
 * <code>log4j.appender.myfile.File=${demo.root}/WEB-INF/demo.log</code>
 * 
 * <p>
 ...
 */
public abstract class Log4jWebConfigurerCustom {
    ...
    private static boolean exposeWebAppRoot(final ServletContext servletContext) {
        ...
        return exposeWebAppRootParam == null
                || Boolean.valueOf(exposeWebAppRootParam);
    }
    ...
    public static void initLogging(final ServletContext servletContext) {
        ...
        // Début modif
        String file = servletContext.getInitParameter("log4j-init-property");
        Properties propFile = new Properties();

        try {
            if (Log4jWebConfigurerCustom.class.getResourceAsStream(file) != null) {
                propFile.load(Log4jWebConfigurerCustom.class
                        .getResourceAsStream(file));
                String log4jInitFile = propFile.getProperty("log4jInitFile");
                location = Log4jWebConfigurerCustom.class.getResource(
                        log4jInitFile).toString();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        if (location != null) {
            ...
            try {
                ...
                servletContext
                        .log("Initializing log4j from [" + location + "]");

                // Check whether refresh interval was specified.
                String intervalString = servletContext
                        .getInitParameter(REFRESH_INTERVAL_PARAM);
